...
{
	Network nt;
	char Message[DEFAULT_BUFLEN] = "yuval shavit";
	char RecvMes[DEFAULT_BUFLEN];
	LPCSTR ip = "127.0.0.1";
	LPCSTR port = "5005";
	SOCKET SocketConnect = INVALID_SOCKET;
	SocketConnect = nt.Connect("127.0.0.1", "5005");
	// Send an initial buffer
	nt.Send(SocketConnect, Message);
	//Recieve message
	nt.Recv(SocketConnect, RecvMes);
	printf("\nRecieved Message: %s\n", RecvMes);
	// Close socket
 ...
 *ptr = NULL,
 ...
 // Resolve the server address and port
 iResult = getaddrinfo((PCSTR)"127.0.0.1", DEFAULT_PORT, &hints, &result);
 if ( iResult != 0 ) {
 printf("getaddrinfo failed with error: %d\n", iResult);
 WSACleanup();
 return 1;
